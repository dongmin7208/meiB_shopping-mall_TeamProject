	...
	<body>
		<div id="wrap">
			<div class='ribbon'>
				...
				<a href='./product/cart"/>'><span>cart</span></a>
				<a href='#'><span>myPage</span></a>
				<a href='support'><span>support</span></a>
			</div>
<c:if test="${user eq null }"> 
			<form action='./member/sign/signIn"/>' method="post" id="loginForm">
...
<c:if test="${user ne null }">
			...
			<form action='./product/main/search"/>' method="post" id="search">
			...
			<br><br>
			<div id="header">
				<img alt="index" src="${pageContext.request.contextPath}/resources/image/logo.png" \>
			</div>
			<div id="container">
				<div id="content">
	<table borde="1">
	<c:forEach var="board" items="${hostBoardList}" varStatus="loop">
		...
		<tr>
			<td colspan="4">
			<button type="button" onClick="location.href='/hostboard/modify?honum=${board.honum}'">수정</button>
 			<button onClick="remove(${board.honum})">삭제</button>
			<button type="button" onClick="location.href='/hostboard/notice/'">목록</button>
			</td>
		</tr>
 	</c:forEach>
 	</table>
